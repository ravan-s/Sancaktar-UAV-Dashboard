...
import 'package:sancaktar_gcs/views/screens/gostergeler/altimeter_gauge.dart';
import 'package:sancaktar_gcs/views/screens/gostergeler/attitude_gauge.dart';
import 'package:sancaktar_gcs/views/screens/gostergeler/compass_gauge.dart';
import 'package:sancaktar_gcs/views/screens/gostergeler/speedometer_gauge.dart';
import 'package:sancaktar_gcs/views/screens/gostergeler/vsi_gauge.dart';
import 'package:sancaktar_gcs/views/screens/gostergeler/yaw_gauge.dart';
import '../../controllers/uav_controller.dart';
import '../../models/uav_model.dart';
import 'log_screen.dart';

// ── Sabit Drone ID listesi ────────────────────────────────────
const _kDrones = <String, String>{
  'alan_tarama': 'ALAN TARAMA',
  'tasiyici': 'TAŞIYICI',
  'insan_takip': 'İNSAN TAKİP',
  'kamikaze': 'KAMİKAZE',
  'nesne_tespit': 'NESNE TESPİT',
};
  ...
  Widget build(BuildContext context) {
    ...
    return Column(
      children: [
        SizedBox(
          height: topH,
          child: _TopBar(ctrl: widget.ctrl, p: _p, sf: sf),
        ),
        Expanded(
          child: Row(
            children: [
              ...
              SizedBox(
                width: fleetW,
                child: _FleetPanel(ctrl: widget.ctrl, sf: sf),
              ),
            ],
          ),
        ),
    ...
    return Container(
      ...
      child: Row(
        children: [
          ...
          // ── 5 Drone Durum Göstergesi ──────────────────────
          _DroneStatusBar(ctrl: widget.ctrl, sf: sf),
  ...
  List<Widget> _menuItems(double sf, double fs) {
    ...
    return items
        .map(
          (it) => GestureDetector(
            onTap: () {
              if (it.$1 == 'LOG') Get.to(() => const LogScreen());
            },
            child: Container(
...
// ================================================================
//  5 DRONE DURUM ŞERİDİ  (TopBar içine gömülü)
// ================================================================
class _DroneStatusBar extends StatelessWidget {
  final UavController ctrl;
  final double sf;
  const _DroneStatusBar({required this.ctrl, required this.sf, super.key});
    ...
    return Obx(() {
      return Container(
        ...
        child: Row(
          ...
          children: _kDrones.entries.map((e) {
            final isOnline =
                ctrl.uavList.containsKey(e.key) &&
                (ctrl.uavList[e.key]?.isOnline ?? false);
            final isSelected = ctrl.selectedUavId.value == e.key;
            return GestureDetector(
              onTap: () => ctrl.selectUav(e.key),
              child: Container(
                margin: EdgeInsets.only(
                  left: e.key == _kDrones.keys.first ? 0 : 8 * sf,
                ),
                ...
                decoration: BoxDecoration(
                  ...
                  border: isSelected
                      ? Border.all(color: _C.cyan.withOpacity(0.4), width: 1)
                      : null,
                ),
                child: Row(
                  mainAxisSize: MainAxisSize.min,
                  children: [
                    // Kırmızı / Yeşil nokta
                    Container(
                      width: (7 * sf).clamp(5, 9),
                      height: (7 * sf).clamp(5, 9),
                      decoration: BoxDecoration(
                        shape: BoxShape.circle,
                        color: isOnline ? _C.green : _C.red,
                        boxShadow: isOnline
                            ? [
                                BoxShadow(
    ...
    return Expanded(
      child: Container(
        ...
        child: Column(
          children: [
            Padding(
              padding: EdgeInsets.only(top: 6 * sf, bottom: 2 * sf),
              child: Text(
                label,
                style: TextStyle(
                  color: Colors.white.withOpacity(0.8),
                  fontSize: (9 * sf).clamp(8, 13),
                  fontWeight: FontWeight.bold,
                  letterSpacing: 1.5,
                  fontFamily: 'monospace',
                ),
              ...
              return ListView(
                ...
                children: _kDrones.keys.map((id) {
                  ...
                  return _DroneCard(
                    ...
                    selected: sel,
                    sf: sf,
                    onTap: uav != null ? () => ctrl.selectUav(id) : null,
                  );
                }).toList(),
              );
            }),
...
// ================================================================
//  DRONE KARTI  (label parametresi eklendi, uav nullable)
// ================================================================
class _DroneCard extends StatelessWidget {
  final String id;
  final String label;
  final UavModel? uav; // null → hiç veri gelmemiş
  final bool selected;
  final double sf;
  ...
  @override
  Widget build(BuildContext context) {
    final online = uav?.isOnline ?? false;
    final col = selected ? _C.cyan : (online ? _C.amber : _C.grey);
    ...
    return GestureDetector(
      ...
      child: Container(
        ...
        decoration: BoxDecoration(
          ...
          border: Border.all(
            color: selected ? _C.cyan.withOpacity(0.5) : _C.border,
            width: 1.5,
          ),
        ...
        child: Column(
          ...
          children: [
            ...
            Row(
              children: [
                _tag(
                  online ? 'AKTİF' : 'ÇEVRIMDIŞI',
                  online ? _C.green : _C.red,
                  sf,
                  fs,
                ),
                if (uav != null) ...[
                  SizedBox(width: 4 * sf),
                  _tag(uav!.flightMode, _C.dim, sf, fs),
                ],
              ],
            ),
            if (uav != null) ...[
              ...
              Row(
                children: [
                  ...
                  Text(
                    '${uav!.altitude.toStringAsFixed(0)}m',
                    style: TextStyle(
                      color: _C.cyan,
                      fontSize: (fs * 0.85).clamp(8, 11),
                      fontWeight: FontWeight.bold,
              ...
              SizedBox(height: 4 * sf),
              Text(
                'Jetson / Raspberry bekleniyor',
                style: TextStyle(
                  color: _C.dim,
                  fontSize: (fs * 0.8).clamp(7, 10),
                  fontFamily: 'monospace',
                ),
              ),
            ],
  ...
  Widget _tag(String t, Color c, double sf, double fs) => Container(
        ...
        return Row(
          children: [
            ...
            _StatCard(
              ...
              valColor: p.verticalSpeed >= 0 ? _C.green : _C.red,
              sf: sf,
            ),
            SizedBox(width: 6 * sf),
            _StatCard(
              ...
              unit: uav == null
                  ? ''
                  : '${uav.battery_volt.toStringAsFixed(1)} V',
              valColor: uav == null
                  ? _C.grey
            ...
            _StatCard(
              ...
              valColor: p.gpsFix == 3 ? _C.green : _C.red,
              sf: sf,
            ),
            SizedBox(width: 6 * sf),
            _StatCard(
              ...
              child: Column(
                ...
                children: [
                  Text(
                    p.lat != null ? '${p.lat!.toStringAsFixed(5)}°N' : '--',
                    style: TextStyle(
                      color: _C.white,
                      fontFamily: 'monospace',
                  ...
                  Text(
                    p.lon != null ? '${p.lon!.toStringAsFixed(5)}°E' : '--',
                    style: TextStyle(
                      color: _C.white,
                      fontFamily: 'monospace',
                      fontSize: (11 * sf).clamp(9, 14),
              ...
              child: Container(
                ...
                child: Column(
                  ...
                  children: [
                    ...
                    Text(
                      p.ctrl.selectedUavId.value.isEmpty
                          ? '--'
...
// ================================================================
//  INSTRUMENT PAINTERS (CustomPainter) — değişmedi
// ================================================================
...
void _tick(
  ...
  c.drawLine(
    ...
    Offset(
      ct.dx + r * (0.92 - lenRatio) * math.cos(a),
...
void _valuebox(
  ...
  final rr = RRect.fromRectAndRadius(
    Rect.fromCenter(
      center: Offset(ct.dx, ct.dy + r * 0.32),
      width: r * 0.78,
      height: r * 0.24,
    ),
...
class AirspeedPainter extends CustomPainter {
  ...
  void paint(Canvas canvas, Size size) {
    ...
    void arc(double from, double to, Color col) {
      ...
      canvas.drawArc(
        Rect.fromCircle(center: ct, radius: r * 0.86),
        sa,
        sw,
        false,
        p,
...
class AttitudePainter extends CustomPainter {
  ...
  void paint(Canvas canvas, Size size) {
    final ct = Offset(size.width / 2, size.height / 2);
    final r = size.width / 2;
    final clipOval = ui.Path()
      ..addOval(Rect.fromCircle(center: ct, radius: r * 0.92));
    ...
    canvas.drawRect(
      Rect.fromLTRB(-r, -r * 2 + py, r, py),
      Paint()..color = const Color(0xFF1A5FA8),
    );
    canvas.drawRect(
      Rect.fromLTRB(-r, py, r, r * 2),
    ...
    for (int a in [-60, -45, -30, -20, -10, 0, 10, 20, 30, 45, 60]) {
      ...
      canvas.drawLine(
        Offset(
          ct.dx + r * 0.92 * math.cos(rad),
          ct.dy + r * 0.92 * math.sin(rad),
        ),
        Offset(
          ct.dx + r * (major ? 0.80 : 0.85) * math.cos(rad),
    ...
    final tri = ui.Path()
      ..moveTo(
        ct.dx + r * 0.86 * math.cos(rRad),
        ct.dy + r * 0.86 * math.sin(rRad),
      )
      ..lineTo(
        ct.dx + r * 0.77 * math.cos(rRad - 0.08),
    ...
    final pp = Paint()
      ...
      ..strokeCap = StrokeCap.round;
    canvas.drawLine(
      Offset(ct.dx - r * 0.4, ct.dy),
      Offset(ct.dx - r * 0.15, ct.dy),
      pp,
    );
    canvas.drawLine(
      Offset(ct.dx + r * 0.15, ct.dy),
...
class AltimeterPainter extends CustomPainter {
  ...
  void paint(Canvas canvas, Size size) {
    ...
    _needle(
      ...
      const Color(0xFFECF0F1).withOpacity(0.5),
      r * 0.018,
    );
    _needle(
      canvas,
...
class YawPainter extends CustomPainter {
  ...
  void paint(Canvas canvas, Size size) {
    ...
    final tri = ui.Path()
      ..moveTo(
        ct.dx + r * 0.8 * math.cos(rRad),
        ct.dy + r * 0.8 * math.sin(rRad),
      )
      ..lineTo(
        ct.dx + r * 0.71 * math.cos(rRad - 0.07),
    ...
    _label(
      ...
      TextStyle(
        color: armed ? const Color(0xFFE53935) : const Color(0xFF00E676),
        fontSize: r * 0.11,
        fontWeight: FontWeight.bold,
        fontFamily: 'monospace',
      ),
  ...
  void _drawDrone(Canvas canvas, Offset ct, double r) {
    ...
    canvas.drawRect(
      Rect.fromCenter(center: ct, width: r * 0.5, height: r * 0.22),
      bp,
    );
    final ap = Paint()
      ..color = const Color(0xFFECF0F1)
    ...
    for (final d in [
      ...
      canvas.drawLine(
        ...
        Offset(ct.dx + d[1].dx * r, ct.dy + d[1].dy * r),
        ap,
      );
      canvas.drawCircle(
        Offset(ct.dx + d[1].dx * r, ct.dy + d[1].dy * r),
  ...
  void paint(Canvas canvas, Size size) {
    ...
    for (int i = 0; i < 36; i++) {
      ...
      canvas.drawLine(
        ...
        Offset(
          r * (major ? 0.76 : 0.82) * math.cos(a),
          r * (major ? 0.76 : 0.82) * math.sin(a),
        ),
        Paint()
          ..color = major ? const Color(0xFFECF0F1) : const Color(0xFF546E7A)
